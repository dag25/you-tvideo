const API_KEY = 'AIzaSyAiY-BnEJCUPjqBEjcwA1Uhv-jeGX2IXEg';
...
const createListVideo = (videos, titleText, pagination) => {
		...
		li.innerHTML = `
			<article class="video-card">
				<a class="video-card__link" href="#/video/${id}">
					...
					${convertISOToReadableDuration(video.contentDetails.duration)}
					</p>`
							: ''
					}
				</a>
	...
	if (pagination) {
		...
		if (pagination.prev) {
			...
			arrowPrevPrevPrev.classList.add('pagination__arrow');
			arrowPrevPrev.textContent = 'Предыдущая страница';
				...
				<div class=" video__content">
					...
					<p class="video__info">
						<span class="video__views">
							${parseInt(video.statistics.viewCount).toLocaleString()} просмотр
						</span>
						<span class="video__date">
							Дата премьеры: ${formatDate(video.snippet.publishedAt)}
						</span>
					</p>
